...
SimpleFilteringGUI::SimpleFilteringGUI() {
    ...
    int initialRunType = 2; // 0:naive, 1:Twopass, 2:Local-Naive, 3:?(undefined now), 4:Local-Twopass
...
void SimpleFilteringGUI::updateRunType(int value){
    if (value < 0 || value > 4 || value==mRunType || value == 3 ) return;
